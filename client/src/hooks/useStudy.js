...
export function useStudy() {
  ...
  const run = useCallback(async (query) => {
    ...
    try {
      ...
      // v3.1.1: Normalize demographics shape. If the API returns nested {acs: {...}},
      // flatten canonical fields onto the root so downstream renderers (Section 3/4
      // KPI tiles in PDF/PPTX/XLSX) do not need to know which pipeline wrote them.
      if (demoData && demoData.acs && typeof demoData.acs === 'object') {
        const acs = demoData.acs;
        const canon = [
          'population','popGrowth5yr','households','mhi','mhiYoY','perCapitaIncome',
          'medianAge','avgHouseholdSize','homeownershipRate','povertyRate',
          'vacancyRate','unemploymentRate','commuteInflow','commuteOutflow',
          'bachelorsPlus','graduatePlus','affordableCeiling','ownerOccupied',
          'renterOccupied','totalHousingUnits','popTrend','popTrendScope',
          'popTrendLevel','employmentBySector','totalEmployment'
        ];
        for (const k of canon) {
          if (demoData[k] == null && acs[k] != null) demoData[k] = acs[k];
        }
      }
  ...
  const study = geo ? {
    ...
    swot: analysis?.swot,
    // v3.0 Step 8: surface FRED-derived 5-year forward projections to PDF/PPTX/XLSX renderers
