  ...
  const run = useCallback(async (query) => {
    ...
    try {
      ...
      if (housingData && demoData) {
        ...
        // v2.3: merge ACS tenure + housing-stock count so Dashboard/deliverables see ~10k units, not Redfin inventory
      ...
      if (compData && housingData) {
        if (compData.marketKPIs && compData.marketKPIs.daysOnMarket == null) {
          compData.marketKPIs.daysOnMarket = housingData.medianDOM ?? housingData.daysOnMarket ?? null;
        }
        // also mirror onto top-level for deliverables that read compData.daysOnMarket directly
