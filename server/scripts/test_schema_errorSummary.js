 ...
 *        • Present when rows populated; includes "Appendix A" header, per-row
...
check('pdf.js: OMITS appendix when _schema missing', () => {
  ...
  assert.ok(!html.includes('Appendix A'),           'Appendix A must not appear');
...
check('pdf.js: RENDERS appendix with all expected elements when rows populated', () => {
  ...
  assert.ok(html.includes('Appendix A: Data Quality Report'));
