/**
 * Infinity Markets v1.6 — Express Server Entry (Sprint 5)
 *
 * v1.6: Added ETL route for Redfin/Zillow/Parcl Labs data imports
 */
...
const etlRoute           = require('./routes/etl');
const { router: authRouter, verifyToken } = require('./routes/auth');
const healthRoute        = require('./routes/health');
const debugRoute         = require('./routes/debug');  // v2.6
...
async function start() {
  try {
    ...
    app.use('/api', (req, res, next) => {
      ...
      const publicPaths = ['/auth', '/geocode', '/demographics', '/housing', '/competition', '/analysis', '/health', '/deliverables', '/etl', '/debug'];
    ...
    app.use('/api/debug',         debugRoute);   // v2.6
    ...
    app.listen(PORT, () => {
      console.log(`[server] Infinity Markets v1.6.0 listening on :${PORT}`);
    });
  } catch (err) {
