...
async function start() {
  try {
    ...
    app.get('/api/health', (req, res) => {
      res.json({ status: 'ok', configKeys: Object.keys(config).length, cacheSize: cache.size });
    });
    ...
    app.listen(PORT, () => {
      console.log(`[server] Infinity Markets v1.0 listening on :${PORT}`);
    });
  } catch (err) {
