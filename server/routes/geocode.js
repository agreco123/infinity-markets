...
async function processMatch(match, supabase, res) {
  ...
  // v2.6 Step 4: even though Census address-match usually returns NAME, defend against
  // edge cases where county.NAME is blank (observed in some Western PA results) by
  // cascading to the hardcoded fallback maps.
  const censusCountyName = (county.NAME || '').replace(/\s+County$/i, '') || null;
  const fallbackCountyName = censusCountyName || lookupCountyFallback(stateFips, countyFips);
  const censusSubdivName = subdiv.NAME || null;
  const fallbackSubdivName = censusSubdivName || lookupSubdivFallback(stateFips, countyFips, subdivFips);
...
function lookupCountyFallback(stateFips, countyFips) {
  const key = `${stateFips}_${countyFips}`;
...
function lookupSubdivFallback(stateFips, countyFips, subdivFips) {
  ...
  const key = `${stateFips}_${countyFips}_${subdivFips}`;
...
async function processCoordResult(cr, supabase, res) {
  ...
  // v2.6 Step 4: fallback to hardcoded Western PA/NY maps when Supabase returned nothing
