...
function scoreColor(s) {
  if (s >= 7) return '#004D41';   // Forbes green
  if (s >= 5) return '#B47C10';   // brass-deep
  return '#A82020';               // editorial red
}
function recLabel(score) {
  if (score >= 7.5) return { text: 'STRONG ENTRY',          color: '#FFFFFF', bg: '#004D41' };
  if (score >= 6.0) return { text: 'PROCEED WITH CAUTION',  color: '#3A2A06', bg: '#D3B257' };
  if (score >= 4.5) return { text: 'HOLD — RE-UNDERWRITE',  color: '#7A5A1F', bg: '#F1EAD8' };
  return                       { text: 'PASS',                color: '#FFFFFF', bg: '#7A1B1B' };
}
