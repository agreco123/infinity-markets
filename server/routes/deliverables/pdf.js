...
const { PDF_STYLES_CSS, buildCoverHTML, buildTOCHTML } = require('./pdf-styles');
...
// V41P-1 / W-1 — row-level ASP audit chip. Reads _aspReason/_aspRevEnd/
// _aspDelEnd directly off a builder row (set by V41-10 extractEdgarMetrics
// and forwarded through competition.js edgarFields). Returns '' when no
// _aspReason is set so aligned rows stay visually clean.
function aspChipFromRow(b) {
  if (!b || !b._aspReason) return '';
  const bits = [String(b._aspReason)];
  if (b._aspRevEnd || b._aspDelEnd) {
    bits.push('rev=' + (b._aspRevEnd || '?') + ' \u00B7 delivered=' + (b._aspDelEnd || '?'));
  }
  const tip = ' title="' + esc(bits.join(' \u00B7 ')) + '"';
  return ' <span class="prov-chip prov-error"' + tip + '>ERR</span>';
}
// V41P-2 / W-2 — row-level builder price-fallback chip. When competition.js
// backfilled an EDGAR-only builder's priceMin/Max from market-wide listing
// quartiles (no community match, no ASP), it sets _priceFallbackReason.
// We surface that as a prov-error chip so the Price Range cell reads as
// 'range present BUT labeled as proxy' rather than silent fabrication.
function builderFallbackChip(b) {
  if (!b || !b._priceFallbackReason) return '';
  const bits = [String(b._priceFallbackReason)];
  if (b._priceFallbackN) bits.push('n=' + b._priceFallbackN + ' listings');
  const tip = ' title="' + esc(bits.join(' \u00B7 ')) + '"';
  return ' <span class="prov-chip prov-error"' + tip + '>PROXY</span>';
}
...
function buildPDFHTML(study) {
  ...
  p('<title>Infinity Markets — ' + esc(study.targetArea || '') + '</title>');
  p('<style>' + PDF_STYLES_CSS + '</style>');
  p('</head><body>');

  // ── Editorial cover (v4.2.0) ──
  {
    const fipsLine = [g && g.stateFips, g && g.countyFips, g && g.subdivFips].filter(Boolean).join('-') || '—';
    const cbsaLine = (g && (g.cbsaName || g.cbsa)) ? safeStr(g.cbsaName || g.cbsa) : '—';
    const verdictKind = wtdAvg >= 7.5 ? 'go' : wtdAvg >= 6 ? 'conditional' : (scorecard.length ? 'no-go' : null);
    const scoreText = scorecard.length ? ('Composite ' + wtdAvg.toFixed(1) + ' / 10 · ' + (rec.text || '')) : null;
    p(buildCoverHTML({
      targetArea: study.targetArea || (g && g.name) || '—',
      fips: fipsLine,
      cbsa: cbsaLine,
      dateLine: dateStr,
      verdict: verdictKind,
  ...
  // ── Editorial table of contents (v4.2.0) ──
  {
    const tocItems = [
      { num: '01', label: 'Executive summary' },
      { num: '02', label: 'Market area' },
      { num: '03', label: 'Demographics' },
      { num: '04', label: 'Housing market' },
      { num: '05', label: 'Building permits' },
      { num: '06', label: 'Competitive landscape' },
      { num: '07', label: 'Absorption & demand' },
      { num: '08', label: 'Pricing strategy' },
      { num: '09', label: 'Land economics' },
      { num: '10', label: 'Pro forma scenarios' },
      { num: '11', label: 'Regulatory environment' },
      { num: '12', label: 'Market scorecard' },
      { num: '13', label: 'SWOT analysis' },
      { num: '14', label: 'Data sources & methodology' },
      { num: 'A',  label: 'Source manifest · Provenance trail' },
      { num: 'B',  label: 'Data quality report (when present)' },
    ];
    p(buildTOCHTML(tocItems));
  }

  // ── 1. Executive Summary ──
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">01</div><div class="label"><span class="eyebrow">Section 01</span><h2>Executive Summary</h2></div></div>');
  ...
  // ── 2. Market Area ──
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">02</div><div class="label"><span class="eyebrow">Section 02</span><h2>Market Area</h2></div></div>');
  ...
  // ── 3. Demographics ──
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">03</div><div class="label"><span class="eyebrow">Section 03</span><h2>Demographics</h2></div></div>');
...
function buildPDFHTML_Part2(study, d, h, comp, abs, pricing, land, proforma, reg, scorecard, swot, g, wtdAvg, rec, sources) {
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">04</div><div class="label"><span class="eyebrow">Section 04</span><h2>Housing Supply &amp; Inventory</h2></div></div>');
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">05</div><div class="label"><span class="eyebrow">Section 05</span><h2>Building Permits &amp; Construction Activity</h2></div></div>');
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">06</div><div class="label"><span class="eyebrow">Section 06</span><h2>Competitive Landscape</h2></div></div>');
  p('<div class="kpi-grid four">');
  ...
  if (comp.builders && comp.builders.length) {
    ...
    comp.builders.slice(0, 15).forEach(b => {
      ...
      p('<tr><td>' + esc(safeStr(b.name || b.builder)) + '</td><td>' + commCell + '</td><td>' + esc(safeStr(range)) + builderFallbackChip(b) + '</td><td>' + avgSqftCell + '</td><td>' + psf + '</td></tr>');
    });
    p('</table>');
  }
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">07</div><div class="label"><span class="eyebrow">Section 07</span><h2>Absorption Analysis</h2></div></div>');
  p('<div class="kpi-grid">');
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">08</div><div class="label"><span class="eyebrow">Section 08</span><h2>Pricing Strategy</h2></div></div>');
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">09</div><div class="label"><span class="eyebrow">Section 09</span><h2>Land Acquisition</h2></div></div>');
  p('<div class="kpi-grid four">');
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">10</div><div class="label"><span class="eyebrow">Section 10</span><h2>Pro Forma Financials</h2></div></div>');
  ...
  if (ebMarks.length) {
    ...
    ebMarks.slice(0, 10).forEach(b => {
      ...
      p('<tr><td>' + esc(safeStr(b.name || b.builder)) + '</td>' +
        ...
        '<td class="text-right">' + (asp ? '$' + fmt(asp) : '—') + aspChipFromRow(b) + '</td>' +
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">11</div><div class="label"><span class="eyebrow">Section 11</span><h2>Regulatory Environment</h2></div></div>');
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">12</div><div class="label"><span class="eyebrow">Section 12</span><h2>Market Scorecard</h2></div></div>');
  ...
  // ── Section 13: SWOT ──
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">13</div><div class="label"><span class="eyebrow">Section 13</span><h2>SWOT Analysis</h2></div></div>');
  ...
  p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">14</div><div class="label"><span class="eyebrow">Section 14</span><h2>Data Sources &amp; Methodology</h2></div></div>');
  ...
  // ── Appendix A: Source Manifest (v4.2.0 — Output E inline) ──
  // Walks every _env envelope on the study + its sub-buckets. One row per
  // canonical field with provenance class, value, source URL, fetched-at,
  // confidence. Always emitted (replaces the concept of a separate Output E
  // file — the directive's Output D + E are now one PDF, the manifest as
  // an editorial appendix at the end).
  {
    p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">A</div><div class="label"><span class="eyebrow">Appendix A</span><h2>Source Manifest · Provenance Trail</h2></div></div>');
    p('<p class="lede">Every value rendered in this report carries a provenance classifier. This appendix is the audit trail: per-field, the canonical name, the rendered value, the provenance class (measured / derived / modeled / LLM / missing), the upstream source URL, when it was fetched, and the confidence band.</p>');
    const manifestRows = [];
    ...
    visit(study && study._env, '');
    ['geo','demographics','housing','competition','analysis'].forEach(b => {
      if (study && study[b] && study[b]._env) visit(study[b]._env, b);
    });
    if (manifestRows.length === 0) {
      p('<p class="narrative" style="font-style:italic;color:#6B6B66;">No <code>_env</code> envelopes are persisted on this study yet. Provenance was inferred at render time from the canonical fields. Future regenerations will surface the full manifest here once the schema migration 004 columns are populated upstream.</p>');
    } else {
      const counts = {};
      manifestRows.forEach(r => { counts[r.provenance] = (counts[r.provenance] || 0) + 1; });
      const order = ['measured','derived','modeled','llm','missing'];
      const summary = order
        .filter(k => counts[k])
        .map(k => '<span class="prov-' + k + '" style="margin-left:0;margin-right:6pt;">' + k + ' · ' + counts[k] + '</span>')
        .join('');
      p('<div style="margin:12pt 0 16pt;display:flex;flex-wrap:wrap;align-items:center;gap:6pt;">' + summary + '<span style="font-family:Inter,sans-serif;font-size:9pt;color:#6B6B66;margin-left:4pt;">' + manifestRows.length + ' fields tracked</span></div>');
      p('<table class="manifest-table">');
      p('<tr><th style="width:24%">Field</th><th style="width:11%">Provenance</th><th style="width:18%" class="text-right">Value</th><th style="width:8%" class="text-center">Conf.</th><th style="width:13%">Fetched</th><th>Source</th></tr>');
      manifestRows.forEach(r => {
        let valStr = '—';
        if (r.value != null) {
          if (typeof r.value === 'number') {
            valStr = (Math.abs(r.value) >= 1000) ? r.value.toLocaleString('en-US', { maximumFractionDigits: 0 }) : String(r.value);
          } else {
            valStr = esc(safeStr(r.value));
            if (valStr.length > 56) valStr = valStr.slice(0, 56) + '…';
          }
        }
        const fetched = r.fetched_at
          ? (function(){ try { return new Date(r.fetched_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }); } catch(_) { return r.fetched_at; } })()
          : '—';
        const urlCell = r.source_url
          ? '<a class="url" href="' + esc(r.source_url) + '">' + esc(r.source_url.length > 48 ? r.source_url.slice(0, 48) + '…' : r.source_url) + '</a>'
          : '—';
        p('<tr>' +
          '<td><code>' + esc(r.field) + '</code></td>' +
          '<td><span class="prov-' + r.provenance + '" style="margin-left:0;">' + r.provenance + '</span></td>' +
          '<td class="text-right">' + valStr + '</td>' +
          '<td class="text-center" style="color:#6B6B66;">' + esc(safeStr(r.confidence || '—')) + '</td>' +
          '<td style="color:#6B6B66;">' + esc(fetched) + '</td>' +
          '<td>' + urlCell + '</td>' +
          '</tr>');
      });
      p('</table>');
      p('<div class="disclaimer"><strong>How to read this table.</strong> Provenance taxonomy: <em>measured</em> means a direct observation from a primary source; <em>derived</em> is computed from one or more measured inputs; <em>modeled</em> indicates a fitted curve or projection (look for the R² gate footnote in the relevant section); <em>LLM</em> is a synthesized text annotation, never a measured value; <em>missing</em> is rendered as an em-dash with no derivation. Confidence bands: <em>high</em> ≥ 95% certainty interval, <em>medium</em> 80–95%, <em>low</em> &lt; 80% or unknown sample size.</div>');
    }
  }
  ...
  if (_rows.length > 0) {
    p('<div class="page-break" style="margin-top:18pt"></div><div class="section-mast"><div class="num">B</div><div class="label"><span class="eyebrow">Appendix B</span><h2>Data Quality Report</h2></div></div>');
