...
router.get('/full', async (req, res) => {
  ...
  try {
    const tables = [
      'api_usage', 'builder_profiles', 'building_permits', 'census_demographics',
      'communities', 'fips_lookup', 'fred_series_ref', 'fred_timeseries',
      'lodes_commute', 'proforma_scenarios', 'redfin_monthly', 'scorecard',
      'source_log', 'studies', 'zillow_zhvi',
    ];
...
router.get('/geocode', async (req, res) => {
  ...
  async function tryStrategy(name, url) {
    const start = Date.now();
  ...
  // Strategy 4: Structured city,state
  if (!match) {
    const parts = q.split(',').map(s => s.trim());
    if (parts.length >= 2) {
      match = await tryStrategy('Structured city/state',
        `${base}/geographies/address?street=&city=${encodeURIComponent(parts[0])}&state=${encodeURIComponent(parts[1])}&benchmark=Public_AR_Current&vintage=Current_Current&format=json`);
    }
  }
  ...
  results.resolved = !!match;
  results.finalMatch = match || null;
  return res.json(results);
});
...
router.get('/sources', async (req, res) => {
  ...
  await testSource('Census ACS', async () => {
    const acsYear = new Date().getFullYear() - 1;
    ...
    const data = await r.json();
    return { headers: data[0], values: data[1], population: data[1]?.[1], mhi: data[1]?.[2] };
  });
