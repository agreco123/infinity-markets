...
router.get('/full', async (req, res) => {
  ...
  // 4. External API reachability (quick ping, no data)
  results.apis = {};
  const apiTests = [
    ...
    { name: 'Census ACS', url: `https://api.census.gov/data/2023/acs/acs5?get=NAME&for=state:36&key=${config.census_api_key || 'NOKEY'}` },
    { name: 'FRED', url: `https://api.stlouisfed.org/fred/series?series_id=MORTGAGE30US&api_key=${config.fred_api_key || 'NOKEY'}&file_type=json` },
    { name: 'BEA', url: `https://apps.bea.gov/api/data?&UserID=${config.bea_api_key || 'NOKEY'}&method=GetData&datasetname=Regional&TableName=CAINC1&LineCode=1&GeoFips=36029&Year=2023&ResultFormat=json` },
  ];
...
router.get('/sources', async (req, res) => {
  ...
  async function testSource(name, fn) {
    ...
    try {
      const data = await fn();
      results[name] = { status: 'ok', ms: Date.now() - start, hasData: data != null, preview: typeof data === 'object' ? summarize(data) : data };
    } catch (e) {
  ...
  await testSource('Census ACS', async () => {
    ...
    const url = `${censusBase}/${acsYear}/acs/acs5?get=NAME,B01003_001E,B19013_001E&for=county:${countyFips}&in=state:${stateFips}&key=${censusKey}`;
    const r = await fetch(url, { signal: AbortSignal.timeout(10000) });
    if (!r.ok) {
      // Try year-3
      const url2 = `${censusBase}/${acsYear - 1}/acs/acs5?get=NAME,B01003_001E,B19013_001E&for=county:${countyFips}&in=state:${stateFips}&key=${censusKey}`;
      const r2 = await fetch(url2, { signal: AbortSignal.timeout(10000) });
      if (!r2.ok) throw new Error(`${r.status} for ${acsYear}, ${r2.status} for ${acsYear - 1}`);
      const data2 = await r2.json();
      return { year: acsYear - 1, headers: data2[0], values: data2[1], population: data2[1]?.[1], mhi: data2[1]?.[2] };
    }
    const data = await r.json();
    return { year: acsYear, headers: data[0], values: data[1], population: data[1]?.[1], mhi: data[1]?.[2] };
  });

  // Census PEP
  await testSource('Census PEP', async () => {
    const url = `${censusBase}/2023/pep/population?get=POP_2020,DATE_CODE,DATE_DESC&for=county:${countyFips}&in=state:${stateFips}&key=${censusKey}`;
    const r = await fetch(url, { signal: AbortSignal.timeout(10000) });
    if (!r.ok) throw new Error(`${r.status} ${r.statusText}`);
    const data = await r.json();
    return { rows: data.length - 1, sample: data.slice(0, 3) };
  });
  ...
  // FRED unemployment (LAUS county pattern)
  await testSource('FRED Unemployment', async () => {
    ...
    const r = await fetch(url, { signal: AbortSignal.timeout(8000) });
    if (!r.ok) throw new Error(`${r.status} ${r.statusText} (series: ${seriesId})`);
    const data = await r.json();
    return { seriesId, rate: data?.observations?.[0]?.value, date: data?.observations?.[0]?.date };
  });

  // Census BPS (building permits)
  await testSource('Census BPS', async () => {
    const yr = String(new Date().getFullYear() - 2);
    const url = `${censusBase}/${yr}/bps/annperm?get=BLDGS_1UNIT,UNITS_1UNIT&for=county:${countyFips}&in=state:${stateFips}&key=${censusKey}`;
    const r = await fetch(url, { signal: AbortSignal.timeout(10000) });
    if (!r.ok) throw new Error(`${r.status} ${r.statusText} (year ${yr})`);
    const data = await r.json();
    if (!Array.isArray(data) || data.length < 2) throw new Error(`Unexpected response for year ${yr}`);
    return { year: yr, headers: data[0], values: data[1] };
  });
  ...
  await testSource('BEA', async () => {
    ...
    if (!r.ok) throw new Error(`${r.status} ${r.statusText}`);
    const data = await r.json();
    return { rows: data?.BEAAPI?.Results?.Data?.length || 0 };
  });

  // Supabase: Redfin data
  await testSource('Supabase redfin_monthly', async () => {
    if (!zipList.length) throw new Error('No ZIPs provided');
