...
router.get('/full', async (req, res) => {
  ...
  const apiTests = [
    ...
    { name: 'Census BPS (flat file)', url: 'https://www2.census.gov/econ/bps/County/co2023a.txt' },
...
router.get('/sources', async (req, res) => {
  ...
  await testSource('Census ACS', async () => {
    const years = [currentYear - 3, currentYear - 4, currentYear - 5]; // e.g. [2023, 2022, 2021]
  ...
  await testSource('FRED Unemployment', async () => {
    ...
    if (cbsa) {
      try {
        const { data } = await supabase.from('fred_series_ref').select('series_id').eq('cbsa_code', cbsa).eq('category', 'employment').limit(1);
        if (data?.length > 0) seriesId = data[0].series_id;
      } catch (_) {}
    }
    // Strategy 2: LAUS county pattern
    if (!seriesId) seriesId = `LAUCN${stateFips}${countyFips}0000000003A`;
  ...
  // Census BPS — building permits via flat file download
  // (Census BPS API /data/{yr}/bps/annperm does NOT exist)
  await testSource('Census BPS', async () => {
    const years = [currentYear - 2, currentYear - 3, currentYear - 4]; // e.g. [2024, 2023, 2022]
    ...
    for (const yr of years) {
      ...
      // Skip 2 header rows, find matching county
      for (let i = 2; i < lines.length; i++) {
        const cols = lines[i].split(',').map(c => c.trim().replace(/^"|"$/g, ''));
        if (cols.length >= 18 &&
            String(cols[1]).padStart(2, '0') === stPad &&
            String(cols[2]).padStart(3, '0') === coPad) {
          return { year: yr, source: 'flat_file', county: cols[5], sf_units: cols[7], mf_5plus_units: cols[16] };
        }
      }
    }
    throw new Error(`No BPS flat file data for years ${years.join(', ')}`);
  });
