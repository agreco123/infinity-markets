...
async function fetchCensusACS(base, key, stateFips, countyFips, subdivFips) {
  ...
  const url = `${base}/${acsYear}/acs/acs5?get=NAME,${vars}&${geo}&key=${key}`;
  let data;
  try {
    data = await fetchJson(url);
  } catch (_) {
    // Fallback to prior year if current year not yet available
    const fallbackUrl = `${base}/${acsYear - 1}/acs/acs5?get=NAME,${vars}&${geo}&key=${key}`;
    data = await fetchJson(fallbackUrl);
  }

  // Census returns [ [header_row], [value_row] ]
  if (!data || data.length < 2) throw new Error('Empty ACS response');
  const headers = data[0];
  const values = data[1];
  const result = {};
  // Map our friendly keys back from variable codes
