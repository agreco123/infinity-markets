 ...
 */
/** Helper: fetch a Census URL, retry without key if HTML response (invalid key) */
async function censusFetchDemo(url) {
  const resp = await fetch(url, { signal: AbortSignal.timeout(12000) });
  if (!resp.ok) return null;
  const text = await resp.text();
  if (text.trim().startsWith('<')) {
    // Invalid key or non-existent dataset — retry without key
...
async function fetchCensusACS(base, key, stateFips, countyFips, subdivFips) {
  ...
  const yearsToTry = [currentYear - 3, currentYear - 4, currentYear - 5];
  ...
  if (cbsa) {
    try {
      const { data, error } = await supabase
        ...
        .limit(1);
      if (!error && data?.length > 0) seriesId = data[0].series_id;
    } catch (_) {}
