...
async function fetchCensusACS(base, key, stateFips, countyFips, subdivFips) {
  ...
  // ACS 5-Year for year X is released ~Dec of year X+1.
  // In April 2026: 2023 GUARANTEED (released Dec 2024), 2024 MAYBE, 2025 NO.
  // Start from guaranteed year to avoid wasting time on non-existent datasets.
  const yearsToTry = [currentYear - 3, currentYear - 2, currentYear - 4];

  // Guard against subdivFips being literal "null"/"undefined" from query params
  ...
  for (const geo of geoLevels) {
    for (const yr of yearsToTry) {
      try {
        ...
        // Census returns 200 + HTML for non-existent datasets
        ...
        if (data && data.length >= 2) {
          ...
          result._acsYear = yr;
          result._geoLevel = geo.includes('subdivision') ? 'subdivision' : 'county';
          return result;
        }
 ...
 * NOTE: Census may have restructured PEP endpoints. If /pep/population
 * returns HTML (200 + HTML body), we skip gracefully.
 */
...
async function fetchCBP(base, key, stateFips, countyFips) {
  const currentYear = new Date().getFullYear();
  const yearsToTry = [currentYear - 4, currentYear - 3, currentYear - 5]; // e.g. [2022, 2023, 2021]
