...
const { maxAffordablePrice, DEFAULTS: AFFORD_DEFAULTS } = require('../lib/affordability');
const router = express.Router();

// V41-6 — read latest MORTGAGE30US observation from Supabase fred_timeseries.
// Returns { rate, source_url, fetched_at } or null. Rate is DECIMAL (0.068 not 6.8).
async function readLatestFredRate(supa, seriesId) {
  if (!supa) return null;
  try {
    ...
    return {
      ...
      fetched_at: data[0].observation_date,
    };
  } catch (_) { return null; }
}
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    // V41-6 — DTI-based affordability ceiling (replaces legacy MHI * 3.5 heuristic).
    // Uses latest FRED MORTGAGE30US from Supabase cache when available;
    // falls back to AFFORD_DEFAULTS.rate when null. Other inputs use PA-default
    // tax/insurance/DTI/down/term per lib/affordability.js DEFAULTS.
    const mhiVal = acs ? safeNum(acs.mhi) : (cachedCanon.mhi != null ? Number(cachedCanon.mhi) : 0);
    let _dtiRateInfo = null;
    try {
      const supaForRate = (req.app?.locals?.supabase || null);
      _dtiRateInfo = await readLatestFredRate(supaForRate, 'MORTGAGE30US');
    } catch (_) { _dtiRateInfo = null; }
    const _dtiRate = _dtiRateInfo ? _dtiRateInfo.rate : AFFORD_DEFAULTS.rate;
    const _dtiRateProvenance = _dtiRateInfo ? 'measured' : 'derived';
    const _dtiResult = mhiVal > 0
      ? maxAffordablePrice({
          mhi: mhiVal,
          rate: _dtiRate,
          taxRate: AFFORD_DEFAULTS.taxRate,
          insRate: AFFORD_DEFAULTS.insRate,
          dtiFrontEnd: AFFORD_DEFAULTS.dtiFrontEnd,
          downPct: AFFORD_DEFAULTS.downPct,
          termYears: AFFORD_DEFAULTS.termYears,
        })
      : null;
    const affordableCeiling = (_dtiResult && _dtiResult.homePrice) ? _dtiResult.homePrice : null;
    const _affordabilityInputs = (_dtiResult && _dtiResult.homePrice) ? {
      rate: _dtiRate,
      rateProvenance: _dtiRateProvenance,
      rateSource: _dtiRateInfo ? 'FRED MORTGAGE30US' : 'v4.1 fallback (no live FRED)',
      rateSourceUrl: _dtiRateInfo ? _dtiRateInfo.source_url : null,
      rateFetchedAt: _dtiRateInfo ? _dtiRateInfo.fetched_at : null,
      taxRate: AFFORD_DEFAULTS.taxRate,
      insRate: AFFORD_DEFAULTS.insRate,
      dtiFrontEnd: AFFORD_DEFAULTS.dtiFrontEnd,
      downPct: AFFORD_DEFAULTS.downPct,
      termYears: AFFORD_DEFAULTS.termYears,
      monthlyPITI: _dtiResult.monthlyPITI,
      monthlyPI: _dtiResult.monthlyPI,
      monthlyTaxes: _dtiResult.monthlyTaxes,
      monthlyInsurance: _dtiResult.monthlyInsurance,
      principal: _dtiResult.principal,
      provenance: _dtiRateProvenance,
    } : null;
    ...
    const payload = {
      ...
      // V41-6 — transparency: full DTI input set and computed breakdown.
      _affordabilityInputs: _affordabilityInputs,
