...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const pick = (computed, canonKey) => {
      if (computed != null && !(typeof computed === 'number' && isNaN(computed))) return computed;
    ...
    const _acsNoteResolved = havingAcs ? (acs && acs._acsNote ? acs._acsNote : null)
                            : havingCanon ? 'Values shown at the county level; live Census API returned no data for the subdivision.'
                            : null;
