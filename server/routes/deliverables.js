...
function buildPDFHTML(study) {
  ...
  try {
    ...
    // Merge canonical back onto study (non-destructive: existing keys preserved).
    for (const k of Object.keys(bare || {})) {
      if (k.startsWith('_')) continue;
      if (study[k] == null) study[k] = bare[k];
      else if (typeof study[k] === 'object' && typeof bare[k] === 'object' && !Array.isArray(bare[k])) {
        study[k] = Object.assign({}, bare[k], study[k]);  // existing wins
      }
    }
