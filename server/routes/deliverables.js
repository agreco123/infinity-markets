/**
 * Infinity Markets v1.6 — Deliverables Route (Sprint 3 Overhaul)
 *
...
router.post('/xlsx', async (req, res) => {
  ...
  try {
    ...
    for (const c of (study.competition?.communities || [])) {
      ws5.addRow([c.name, c.builder, c.product, c.plans, c.sfLow, c.sfHigh, c.priceLow, c.priceHigh, c.psfAvg, c.lotsTotal || c.totalLots, c.lotsRemain || c.lotsRemaining, c.monthlyAbs, c.hoa, c.schoolDist]);
    }
    ...
    if (study.pricing?.stratification?.length) {
      ...
      for (const s of study.pricing.stratification) ws7.addRow([s.segment, s.priceRange, s.psfRange, s.shareOfSales / 100, s.builders]);
    }
    ...
    if (study.proforma?.publicBenchmarks?.length) {
      ...
      for (const b of study.proforma.publicBenchmarks) {
        ws9.addRow([b.builder, b.grossMargin / 100, b.asp, b.cancelRate / 100]);
      }
    }
  ...
  try {
    ...
    if (study.competition?.communities?.length) {
      ...
      for (const c of study.competition.communities.slice(0, 10)) {
        ...
        cRows.push([
          { text: c.name || '', options: { color: txt } },
          { text: c.builder || '', options: { color: mut } },
          { text: c.product || '', options: { color: mut, align: 'center' } },
    ...
    if (study.pricing?.stratification?.length) {
      ...
      for (const s of study.pricing.stratification) {
        pRows.push([
          ...
          { text: s.builders, options: { color: mut } },
        ]);
      }
...
<table>
  ...
  ${d.topEmployers.map(e => `<tr><td>${e.sector || e.name}</td><td class="text-right">${fmt(e.emp || e.employment)}</td></tr>`).join('')}
</table>` : ''}
...
<table>
  ...
  ${comp.communities.map(c => `<tr>
    <td>${c.name || ''}</td><td>${c.builder || ''}</td><td>${c.product || ''}</td>
...
<table>
  ...
  ${(proforma.publicBenchmarks || comp.publicBenchmarks || []).map(b => `<tr>
    <td>${b.builder}</td><td class="text-center">${b.grossMargin}%</td>
...
<table>
  ...
  ${pricing.stratification.map(s => `<tr>
    ...
    <td class="text-center bold">${s.shareOfSales}%</td><td style="font-size:8pt">${s.builders}</td>
  </tr>`).join('')}
</table>` : ''}
...
<table>
  <tr><th>Builder</th><th class="text-center">Gross Margin</th><th class="text-right">ASP</th><th class="text-center">Cancel Rate</th></tr>
  ${proforma.publicBenchmarks.map(b => `<tr><td>${b.builder}</td><td class="text-center">${b.grossMargin}%</td><td class="text-right">$${fmt(b.asp)}</td><td class="text-center">${b.cancelRate}%</td></tr>`).join('')}
</table>` : ''}
