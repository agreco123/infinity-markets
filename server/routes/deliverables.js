...
router.post('/pptx', async (req, res) => {
  ...
  try {
    ...
    const geoKvs = [
      ...
      ['Coordinates', g.lat && g.lng ? `${g.lat}, ${g.lng}` : '—'],
    ];
    ...
    const hKpis = [
      ...
      ['Median DOM', safeStr(h2.medianDOM)], ['Months Supply', safeStr(h2.monthsSupply)],
...
function buildPDFHTML(study) {
  ...
  p('<dt>Coordinates</dt><dd>' + (g.lat && g.lng ? g.lat + ', ' + g.lng : '—') + '</dd>');
  p('</dl>');
...
function buildPDFHTML_Part2(study, d, h, comp, abs, pricing, land, proforma, reg, scorecard, swot, g, wtdAvg, rec, sources) {
  ...
  if (comp.builders && comp.builders.length) {
    ...
    comp.builders.slice(0, 15).forEach(b => {
      const range = (b.priceMin || b.minPrice) && (b.priceMax || b.maxPrice)
        ? '$' + fmt(b.priceMin || b.minPrice) + '–$' + fmt(b.priceMax || b.maxPrice)
        : (b.priceRange || '—');
      p('<tr><td>' + esc(safeStr(b.name || b.builder)) + '</td><td>' + fmt(b.communities || b.communityCount || 0) + '</td><td>' + esc(safeStr(range)) + '</td><td>' + fmt(b.avgSqft) + '</td><td>$' + fmt(b.pricePerSqft) + '</td></tr>');
    });
    p('</table>');
  }
  ...
  if (abs.byPriceBand && abs.byPriceBand.length) {
    ...
    abs.byPriceBand.forEach(b => {
      p('<tr><td>' + esc(safeStr(b.band || b.priceBand)) + '</td><td>' + fmt(b.listings) + '</td><td>' + fmt1(b.salesPerMonth) + '</td><td>' + fmt1(b.monthsSupply) + '</td></tr>');
    });
    p('</table>');
  }
  ...
  if (pricing.targetPriceRange) {
    const pr = pricing.targetPriceRange;
    p('<div class="kpi-grid">');
    p('<div class="kpi"><div class="value green">$' + fmt(pr.min || pr.low) + '</div><div class="label">Target Low</div></div>');
    p('<div class="kpi"><div class="value accent">$' + fmt(pr.mid || pr.median) + '</div><div class="label">Target Mid</div></div>');
    p('<div class="kpi"><div class="value red">$' + fmt(pr.max || pr.high) + '</div><div class="label">Target High</div></div>');
    p('</div>');
  }
  ...
  if (pricing.bandedPricing && pricing.bandedPricing.length) {
    ...
    pricing.bandedPricing.forEach(b => {
      p('<tr><td>' + esc(safeStr(b.tier || b.name)) + '</td><td>' + esc(safeStr(b.sqftRange)) + '</td><td>' + esc(safeStr(b.priceRange)) + '</td><td>$' + fmt(b.pricePerSqft) + '</td><td>' + fmtP(b.share) + '</td></tr>');
    });
    p('</table>');
  }
  ...
  p('<div style="font-size:14pt;color:' + rec.color + ';font-weight:600;margin-top:6px;">' + esc(rec.label) + '</div>');
  p('</div>');
  if (scorecard && scorecard.length) {
    ...
    scorecard.forEach(s => {
      ...
      p('<tr><td>' + esc(safeStr(s.factor || s.name)) + '</td><td style="color:' + scoreColor(sc) + ';font-weight:700;">' + fmt1(sc) + '</td><td>' + fmtP(w) + '</td><td>' + weighted + '</td><td style="font-size:8pt;">' + esc(safeStr(s.rationale || s.notes || '')) + '</td></tr>');
    });
    p('</table>');
  }
  p('</div>');
  ...
  if (sources && sources.length) {
    ...
    sources.forEach(s => {
      p('<tr><td>' + esc(safeStr(s.name || s.source)) + '</td><td>' + esc(safeStr(s.data || s.description || '')) + '</td><td>' + esc(safeStr(s.updated || s.asOf || 'Current')) + '</td></tr>');
    });
    p('</table>');
  }
  p('<h3>Methodology</h3>');
