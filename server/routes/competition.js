...
/**
 * extractBuilder — smart builder name extraction from RapidAPI listing objects.
 */
function extractBuilder(l) {
  // Direct string
  if (typeof l.builder === 'string' && l.builder.trim()) return l.builder.trim();
  // Object with .name
  if (typeof l.builder === 'object' && l.builder?.name) return l.builder.name;

  // Branding name
  const branding = l.branding?.name || l.branding?.listing_agent?.name;
  if (typeof branding === 'string' && branding.trim()) return branding.trim();
