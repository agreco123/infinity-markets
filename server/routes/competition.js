  ...
  if (supabase) {
    try {
      const { data } = await supabase
        ...
        .select('*')
        .or(`study_target.eq.${city}, PA,study_target.eq.${city} township, PA,study_target.eq.${city},PA`);
      if (Array.isArray(data)) seeded = data;
    } catch (_) { /* swallow */ }
  }
...
function extractEdgarMetrics(data, b) {
  ...
  const rev = latestUSD('Revenues')
           || latestUSD('RevenueFromContractWithCustomerExcludingAssessedTax')
           || latestUSD('SalesRevenueNet');
  const gp  = latestUSD('GrossProfit');
  ...
  // Homes delivered: walk all namespaces for /home.*(deliver|settl|closed)/
  for (const ns of Object.keys((data && data.facts) || {})) {
    const block = data.facts[ns];
    for (const concept of Object.keys(block)) {
      if (!/home/i.test(concept)) continue;
      if (!/(deliver|settl|closed)/i.test(concept)) continue;
      ...
      for (const u of Object.keys(units)) {
        ...
        if (cur && cur.val != null && out.homesDelivered == null) {
          out.homesDelivered = cur.val;
        }
      }
    }
  }

  if (rev != null && out.homesDelivered && out.homesDelivered > 0) {
    out.asp = Math.round(rev / out.homesDelivered);
  }

  // Cancellation rate: walk for /cancel/
  for (const ns of Object.keys((data && data.facts) || {})) {
    const block = data.facts[ns];
    for (const concept of Object.keys(block)) {
      ...
      for (const u of Object.keys(units)) {
        ...
        const cur = (pool.length ? pool : units[u]).slice()
          .sort((x, y) => (y.end || '').localeCompare(x.end || ''))[0];
        if (cur && cur.val != null && out.cancelRate == null) {
          const v = Number(cur.val);
  ...
  const meta = pickLatest(gaap.Revenues, 'USD')
            || pickLatest(gaap.RevenueFromContractWithCustomerExcludingAssessedTax, 'USD');
  if (meta) {
    out.filingPeriodEnd = meta.end || null;
    out.filingForm = meta.form || null;
  }
