  ...
  try {
    ...
    await streamLines(REDFIN_URL, (line) => {
      ...
      rows.push({
        ...
        median_ppsf:             safeNum(row.median_ppsf) || safeNum(row.median_price_per_sqft) || safeNum(row.median_sale_ppsf),
        median_list_ppsf:        safeNum(row.median_list_ppsf) || safeNum(row.median_list_price_per_sqft),
        ...
        pending_sales:           safeNum(row.pending_sales) != null ? Math.round(safeNum(row.pending_sales)) : null,
        homes_sold_yoy:          safeNum(row.homes_sold_yoy) || safeNum(row.homes_sold_yoy_pct),
        new_listings_yoy:        safeNum(row.new_listings_yoy) || safeNum(row.new_listings_yoy_pct),
        pending_sales_yoy:       safeNum(row.pending_sales_yoy) || safeNum(row.pending_sales_yoy_pct),
        median_sale_price_yoy:   safeNum(row.median_sale_price_yoy) || safeNum(row.median_sale_price_yoy_pct),
        median_list_price_yoy:   safeNum(row.median_list_price_yoy) || safeNum(row.median_list_price_yoy_pct),
...
/* ═══════════════════════════════════════════════════════════════════════════
   v2.5 — POST /api/etl/seed-nhs
   Trigger NewHomeSource scraper for an array of ZIP codes and upsert into
   public.communities. Response: per-zip summary { zip, found, inserted, updated, errors[] }.
   Body: { zips: ["16066","16046","16002","16033","16059"] }
   ═══════════════════════════════════════════════════════════════════════════ */
router.post('/seed-nhs', async (req, res) => {
  const zips = Array.isArray(req.body?.zips)
    ? req.body.zips.map(z => String(z).trim()).filter(Boolean)
    : [];
  if (!zips.length) {
    return res.status(400).json({ error: 'zips array required' });
  }
  const { supabase, sourceLog } = req.app.locals;
