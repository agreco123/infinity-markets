/**
 * Infinity Markets v1.1 — ETL Route (Sprint 5)
 *
 ...
 *
 * v1.1 hardening (post-validation):
 * - Router-level auth: x-etl-key header must match process.env.ETL_SECRET
 *   (dev without NODE_ENV=production and without ETL_SECRET still open)
 * - Faster upserts: single bulk DELETE per ETL run instead of per-row DELETEs
 *
 * Data sources:
...
/* ── Auth middleware ──────────────────────────────────────────────────────────
   All ETL endpoints (including /status, /seed/*) require the x-etl-key header
   to match process.env.ETL_SECRET. This prevents internet-facing misuse of
   POST /seed/* and expensive streaming ETL runs.
   In non-production without ETL_SECRET set, auth is bypassed for local dev.
*/
router.use((req, res, next) => {
  const expected = process.env.ETL_SECRET;
  if (!expected) {
    if (process.env.NODE_ENV !== 'production') return next();
    console.error('[etl] ETL_SECRET not set in production environment');
    return res.status(500).json({ error: 'ETL_SECRET not configured on server' });
  }
  const provided = req.get('x-etl-key');
  if (provided !== expected) {
    return res.status(401).json({ error: 'Unauthorized: valid x-etl-key header required' });
  }
  return next();
});
...
router.post('/redfin', async (req, res) => {
  ...
  try {
    ...
    // Bulk DELETE once for all affected (zip, period_begin) pairs, then batch INSERT.
    // This replaces the previous per-row DELETE (which issued one HTTP call per row).
    const affectedZips = [...new Set(rows.map(r => r.zip_code))];
    const affectedPeriods = [...new Set(rows.map(r => r.period_begin).filter(Boolean))];
    if (affectedZips.length && affectedPeriods.length) {
      const { error: delErr } = await supabase
        .from('redfin_monthly')
        .delete()
        .in('zip_code', affectedZips)
        .in('period_begin', affectedPeriods);
      if (delErr) console.warn('[etl/redfin] Pre-insert cleanup error:', delErr.message);
    }
...
router.post('/zillow', async (req, res) => {
  ...
  try {
    ...
    // Delete existing Zillow data for these ZIPs in one call (replaces per-ZIP loop)
    {
      const { error: delErr } = await supabase
        .from('zillow_zhvi')
        .delete()
        .eq('region_type', 'zip')
        .in('region_name', [...zipSet]);
      if (delErr) console.warn('[etl/zillow] Pre-insert cleanup error:', delErr.message);
    }
