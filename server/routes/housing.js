...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const _fetchStatus = {};
    for (let i = 0; i < results.length; i++) {
      _fetchStatus[sourceNames[i]] = makeFetchStatus(results[i]);
    }
    ...
    // v2.3: also build a deliverables-friendly permits[] array combining SF + MF by year
    const sfArr = permits?.sf ?? [];
    const mfArr = permits?.mf ?? [];
    const yearMap = new Map();
    for (const r of sfArr) {
      const yr = String(r.yr ?? r.year ?? '').trim();
      if (!yr) continue;
      if (!yearMap.has(yr)) yearMap.set(yr, { year: yr, sf: 0, mf: 0 });
      yearMap.get(yr).sf = parseInt(r.v) || 0;
    }
    for (const r of mfArr) {
      const yr = String(r.yr ?? r.year ?? '').trim();
      if (!yr) continue;
      if (!yearMap.has(yr)) yearMap.set(yr, { year: yr, sf: 0, mf: 0 });
      yearMap.get(yr).mf = parseInt(r.v) || 0;
    }
    const permitsArr = [...yearMap.values()].map(p => ({ ...p, total: p.sf + p.mf }))
      .sort((a, b) => String(a.year).localeCompare(String(b.year)));
    const latestSF = permitsArr.length ? permitsArr[permitsArr.length - 1].sf : null;
    const latestMF = permitsArr.length ? permitsArr[permitsArr.length - 1].mf : null;
    const latestPermitYear = permitsArr.length ? permitsArr[permitsArr.length - 1].year : null;

    const payload = {
      // Note: totalUnits from Redfin is *active for-sale inventory*, not housing stock count.
      // v2.3: rename to inventoryActive; useStudy.js overwrites totalUnits with ACS housing-stock count from demographics.
      inventoryActive: redfin?.totalInventory ?? null,
      totalUnits:      redfin?.totalInventory ?? null,
      vacancyRate:     null,
      medianValue:     redfin?.medianSalePrice ?? null,
      medianHomeValue: redfin?.medianSalePrice ?? null,
      valueGrowthYoY:  redfin?.priceGrowthYoY ?? null,
      medianDOM:       redfin?.medianDOM ?? null,
      daysOnMarket:    redfin?.medianDOM ?? null,
      ...
      permitsMF:       mfArr,
      permits:         permitsArr,
      latestPermitsSF: latestSF,
      latestPermitsMF: latestMF,
      latestPermitYear,
    ...
    payload._fetchStatus = _fetchStatus;
...
async function fetchRedfin(supabase, cache, zipList) {
  ...
  try {
    ...
    // v3.0 Step 4: helper for YoY delta (percent) on any column across 13-month window
    const yoy = (field, latestVal) => {
      if (latestVal === null || latestVal === undefined) return null;
      const periodsAll = [...new Set(data.map(r => r.period_end))].sort();
      if (periodsAll.length < 13) return null;
      const priorPeriod = periodsAll[periodsAll.length - 13];
      const priorRows = data.filter(r => r.period_end === priorPeriod);
      const priorVals = priorRows.map(r => safeNum(r[field])).filter(v => v > 0);
      if (!priorVals.length) return null;
      const priorAvg = priorVals.reduce((a, b) => a + b, 0) / priorVals.length;
      return priorAvg > 0 ? Math.round(((latestVal - priorAvg) / priorAvg) * 1000) / 10 : null;
    };
    ...
    // v3.0 Step 4: widened columns
    const medianListPrice      = avg('median_list_price');
    const medianPpsf           = avg('median_ppsf');        // sale ppsf
    const medianListPpsf       = avg('median_list_ppsf');
    const homesSold            = avg('homes_sold');
    const newListings          = avg('new_listings');
    const pendingSales         = avg('pending_sales');
    const priceDropsPct        = avgDec('price_drops_pct');
    const offMarketTwoWeeksPct = avgDec('off_market_in_two_weeks_pct');

    // YoY deltas
    const priceGrowthYoY       = yoy('median_sale_price', medianSalePrice);
    const medianListPriceYoY   = yoy('median_list_price', medianListPrice);
    const medianPpsfYoY        = yoy('median_ppsf', medianPpsf);
    const medianListPpsfYoY    = yoy('median_list_ppsf', medianListPpsf);
    const homesSoldYoY         = yoy('homes_sold', homesSold);
    const newListingsYoY       = yoy('new_listings', newListings);
    const pendingSalesYoY      = yoy('pending_sales', pendingSales);
    const inventoryYoY         = yoy('inventory', totalInventory);

    // 13-month series (for trend charting / ARIMA downstream)
    const periodsSorted = [...new Set(data.map(r => r.period_end))].sort();
    const seriesFor = (field) => periodsSorted.map(p => {
      const rows = data.filter(r => r.period_end === p);
      const vals = rows.map(r => safeNum(r[field])).filter(v => v > 0);
      const v = vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : null;
      return { period: p, value: v };
    });

    const result = {
      // v2.x surface — unchanged
      medianSalePrice, medianDOM, saleToList, monthsSupply, totalInventory, priceGrowthYoY,
      // v3.0 widened surface
      ...
      series: {
        medianSalePrice: seriesFor('median_sale_price'),
        medianListPrice: seriesFor('median_list_price'),
        homesSold:       seriesFor('homes_sold'),
        newListings:     seriesFor('new_listings'),
        pendingSales:    seriesFor('pending_sales'),
        inventory:       seriesFor('inventory'),
        medianDOM:       seriesFor('median_dom'),
      },
      _schema: 'v3.0-step4',
    };
