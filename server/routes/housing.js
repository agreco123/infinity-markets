...
async function fetchBuildingPermits(base, key, stateFips, countyFips) {
  ...
  // Start from year-2 (most likely latest available) through year-6
  ...
  for (const yr of years) {
    try {
      ...
      if (!resp.ok) continue; // Skip years that don't exist
      const data = await resp.json();
      // Census API sometimes returns 200 with error body
      if (!Array.isArray(data) || data.length < 2) continue;
      const headers = data[0];
      const vals = data[1];
      sf.push({ yr, v: safeNum(vals[headers.indexOf('UNITS_1UNIT')]), name: vals[headers.indexOf('NAME')] || '' });
