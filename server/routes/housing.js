...
async function fetchBuildingPermits(base, key, stateFips, countyFips) {
  ...
  // BPS 2024 annual data released May 2025. 2023 guaranteed. Try newest first.
  const years = [currentYear - 3, currentYear - 2, currentYear - 4, currentYear - 5];

  /** Fetch Census API, skip HTML responses (200+HTML = non-existent dataset) */
  ...
  // Possible MF variable names — Census has used different names across years
  const mfVariants = [
    { bldgs: 'BLDGS_5UNITMORE', units: 'UNITS_5UNITMORE' },
    { bldgs: 'BLDGS_5ORMORE', units: 'UNITS_5ORMORE' },
    { bldgs: 'BLDGS_5PLUSUNITS', units: 'UNITS_5PLUSUNITS' },
  ];

  for (const yr of years) {
    try {
      // SF request — known working variable names
      const sfUrl = `${base}/${yr}/bps/annperm?get=NAME,BLDGS_1UNIT,UNITS_1UNIT&for=county:${countyFips}&in=state:${stateFips}&key=${key}`;
      const sfData = await censusFetch(sfUrl);
      if (!sfData) continue;

      const sfH = sfData[0], sfV = sfData[1];
      sf.push({ yr: String(yr), v: safeNum(sfV[sfH.indexOf('UNITS_1UNIT')]), name: sfV[sfH.indexOf('NAME')] || '' });

      // MF request — try each variable name variant
      let mfVal = 0;
      for (const { bldgs, units } of mfVariants) {
        try {
          const mfUrl = `${base}/${yr}/bps/annperm?get=${bldgs},${units}&for=county:${countyFips}&in=state:${stateFips}&key=${key}`;
          const mfData = await censusFetch(mfUrl);
          if (mfData) {
            const mfH = mfData[0];
            mfVal = safeNum(mfData[1][mfH.indexOf(units)]);
            break; // Found working MF variables
          }
        } catch (_) {}
      }
      mf.push({ yr: String(yr), v: mfVal });
    } catch (_) { /* year unavailable */ }
  }
