...
function deriveBandsFromListings(housing) {
  ...
  // salesPerMonth per band = (inventory * weight) / monthsSupply, defaulting
  // monthsSupply to 6 (balanced market) if we don't have it.
  const supply = monthsSupply && monthsSupply > 0 ? monthsSupply : 6;

  return tiers.map(t => {
    const listings = Math.round(inventory * t.weight);
    const bandMonthsSupply = supply;
    const salesPerMonth = +(listings / Math.max(bandMonthsSupply, 1)).toFixed(2);
    return {
      name: t.name,
      tier: t.name,
      listings,
      salesPerMonth,
      monthsSupply: +bandMonthsSupply.toFixed(1),
      communityCount: null,
    ...
    if (!hasBands) {
      ...
      if (!derived || !derived.length || derived.every(b => !b.listings && !b.salesPerMonth)) {
        ...
        if (listingsDerived && listingsDerived.length) {
          ...
          console.log('[enrich v2.6 Step 7] absorption.byPriceBand derived from housing listings (competition empty)');
        }
      } else {
