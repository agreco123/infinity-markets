...
const { normalizeStudy, validateStudy, enforceSchema, SCHEMA_VERSION } = require('../lib/studySchema');
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // v4.1 (V41-3 / H-8): surface compact errorSummary onto the study
      // envelope so deliverables/pdf.js can render a Data Quality block
      // and ops tooling can ping a single string. enforceSchema folds in
      // validateStudy's missing-required errors + any unknown fields and
      // returns { valid, unknown, missingRequired, errorSummary, strict }.
      try {
        const enforced = enforceSchema(canon || { analysis }, {
          context: (targetArea && targetArea.name) ? ('analysis:' + targetArea.name) : 'analysis',
        });
        analysis._schema.errorSummary   = enforced.errorSummary;
        analysis._schema.unknownFields  = enforced.unknown;
        analysis._schema.missingRequired = enforced.missingRequired;
      } catch (enfErr) {
        console.warn('[analysis v4.1] enforceSchema failed:', enfErr.message);
      }
      // V41-5 (H-3): fold enrich-time errors onto _schema so the renderer
      // can surface them as error chips instead of silently rendering blanks.
      if (Array.isArray(analysis._enrichErrors) && analysis._enrichErrors.length) {
        analysis._schema.enrichErrors = analysis._enrichErrors;
        console.warn('[analysis v4.1 H-3] enrich errors:',
          analysis._enrichErrors.map(e => e.bucket + '.' + e.field + ':' + e.cause).join(', '));
        // Don't leave the raw copy — it's already on _schema.
        delete analysis._enrichErrors;
      }
      if (!val.valid) {
    ...
    // v4.1 (V41-2 / H-1): surface per-source fetch outcomes on the study
    // envelope. Each upstream sub-payload (demographics/housing/competition)
    // attaches its own _fetchStatus from server/routes/*.js; merge into one.
    const _fetchStatus = Object.assign({},
      (demographics && demographics._fetchStatus) || {},
      (housing && housing._fetchStatus)           || {},
      (competition && competition._fetchStatus)   || {}
    );
    if (Object.keys(_fetchStatus).length) analysis._fetchStatus = _fetchStatus;
...
function enrichAnalysisForDeliverables(a, housing, competition) {
  ...
  // v4.1 (V41-5 / H-3) — fail-loud enrich. Accumulate per-field errors
  // when a cascade AND its fallback both yield nothing, so the renderer
  // can show an error chip + em-dash instead of an empty block.
  const _enrichErrors = [];
  const _pushEnrichErr = (bucket, field, cause, detail) => {
    _enrichErrors.push({
  ...
  if (a.absorption) {
    ...
    if (!hasBands) {
      ...
      } else if (!Array.isArray(ab.byPriceBand) || ab.byPriceBand.length === 0) {
        // V41-5: both cascade paths empty; emit a loud error.
        _pushEnrichErr('absorption', 'byPriceBand', 'cascade_empty',
          'Claude returned no bands AND competition-derived AND listings-derived both empty');
      }
    }
    ...
    if (Array.isArray(ab.byPriceBand)) {
      ...
      // V41P-7 / W-7 — KILL the 'Claude fabricates flat 12-month supply'
      // LAW #6 violation. Recompute monthsSupply from listings/salesPerMonth
      // whenever both are numeric positive. When they're missing AND Claude's
      // monthsSupply is identical across every band (the 'flat 12' smell),
      // coerce to null so the renderer em-dashes honestly instead of cloning
      // a fabricated default.
      const bands = ab.byPriceBand;
      const claudeValues = bands.map(b => b.monthsSupply).filter(v => v != null);
      const allIdentical = claudeValues.length >= 2
        && claudeValues.every(v => Math.abs(v - claudeValues[0]) < 0.01);
      const allMissingComputeInputs = bands.every(b =>
        !(Number.isFinite(b.listings) && b.listings > 0
          && Number.isFinite(b.salesPerMonth) && b.salesPerMonth > 0));
      // V41P-7 (iter 2): round with Math.round(x*10)/10 — .toFixed(1) rounds
      // IEEE-754 edge cases unexpectedly (e.g. (85/7.1).toFixed(1) === '12.0'
      // because 11.97 stores as 11.97000...63). Always prefer computed value when
      // raw inputs present. Audit trail preserves LLM's opinion for transparency.
      const round1 = x => Math.round(x * 10) / 10;
      for (const b of bands) {
        const canCompute = Number.isFinite(b.listings) && b.listings > 0
          && Number.isFinite(b.salesPerMonth) && b.salesPerMonth > 0;
        if (canCompute) {
          const computed = round1(b.listings / b.salesPerMonth);
          if (b.monthsSupply !== computed) {
            b._monthsSupplyOriginal = b.monthsSupply;
            b.monthsSupply = computed;
            b._monthsSupplySource = 'computed_from_listings';
          }
        } else if (allIdentical && allMissingComputeInputs) {
          // Flat-across-bands + no inputs to verify → Claude fabrication.
          b._monthsSupplyOriginal = b.monthsSupply;
          b.monthsSupply = null;
          b._monthsSupplySource = 'null_flat_fabrication';
        }
      }
    }
  ...
  if (a.pricing) {
    ...
    // V41-5: fail loud if pricing arrived with no stratification AND no
    // targetHomePrice AND no bandedPricing — we have literally nothing to
    // render in Section 8.
    const _hasStrat = Array.isArray(pr.stratification) && pr.stratification.length > 0;
    const _hasTHP   = pr.targetHomePrice && (pr.targetHomePrice.low != null || pr.targetHomePrice.mid != null || pr.targetHomePrice.high != null);
    const _hasBanded= Array.isArray(pr.bandedPricing) && pr.bandedPricing.length > 0;
    if (!_hasStrat && !_hasTHP && !_hasBanded) {
      _pushEnrichErr('pricing', 'stratification', 'all_sources_missing',
        'pricing has no stratification, no targetHomePrice, and no bandedPricing');
    }
  ...
  if (a.land) {
    ...
    // V41P-9 / W-9 — normalize landCostPctOfRevenue to a percent scalar.
    // Claude sometimes emits a decimal fraction (0.18 = 18%) via lotToHomeRatio;
    // when the fallback fires from that field, the value arrives < 1 and pdf.js
    // fmtP renders it as 0.18%. Heuristic: a valid lot-to-home ratio is
    // structurally always < 50% (typical 8-30%). If value > 0 and < 1, treat
    // as decimal fraction and scale ×100; if value > 1 and < 60, treat as
    // already-percent. Values >= 60 are suspicious and leave alone.
    if (Number.isFinite(Number(ln.landCostPctOfRevenue))) {
      const v = Number(ln.landCostPctOfRevenue);
      if (v > 0 && v < 1) {
        ln.landCostPctOfRevenue = +(v * 100).toFixed(1);
        ln._landCostPctSource = 'normalized_from_fraction';
      } else if (v >= 1 && v < 60) {
        ln.landCostPctOfRevenue = +v.toFixed(1);
      }
    }
  ...
  if (a.proforma) {
    ...
    if (!base && (!pf.scenarios || pf.scenarios.length === 0)) {
      _pushEnrichErr('proforma', 'scenarios', 'no_base_scenario',
        'pf.scenarios missing or empty; cannot derive revenue/margin line items');
    }
    if (base) {
  ...
  // V41-5: if scorecard is empty but we had enough upstream data to score,
  // emit an error chip rather than silently render a blank §12.
  if ((!Array.isArray(a.scorecard) || a.scorecard.length === 0)) {
    const _upstreamHadData = !!(
      (housing && (housing.totalUnits || housing.medianHomeValue)) ||
      (competition && (competition.activeListings || (Array.isArray(competition.builders) && competition.builders.length)))
    );
    if (_upstreamHadData) {
      _pushEnrichErr('scorecard', 'scorecard', 'empty_despite_inputs',
        'scorecard[] is empty but upstream housing/competition had signal');
    }
  }

  // V41-5: expose accumulated errors on the enriched object so the hot path
  // can mirror them onto a._schema.enrichErrors.
  if (_enrichErrors.length) {
    a._enrichErrors = _enrichErrors;
  }
}
