...
function enrichAnalysisForDeliverables(a, housing, competition) {
  ...
  if (a.absorption) {
    ...
    const hasBands = Array.isArray(ab.byPriceBand) && ab.byPriceBand.length > 0;
    if (!hasBands) {
      const derived = deriveBandsFromCompetition(competition, ab);
    ...
    if (Array.isArray(ab.byPriceBand)) {
      ab.byPriceBand = ab.byPriceBand.map(b => ({
        ...b,
        name: b.name || b.tier || b.band || null,
        tier: b.tier || b.name || b.band || null,
        listings: safeNumA(b.listings ?? b.active ?? b.activeListings),
  ...
  if (a.pricing) {
    ...
    // v2.5: derive targetHomePrice {low, mid, high} from stratification midpoints
    // rather than reading targetPriceRange's extremes (which were Luxury band ceilings).
    if (!pr.targetHomePrice && Array.isArray(pr.stratification) && pr.stratification.length) {
      const mids = pr.stratification
        .map(s => {
          const r = parsePriceRange(s.priceRange);
      ...
      if (mids.length) {
        ...
        const mid  = sorted.length >= 3
                   ? sorted[Math.floor(sorted.length / 2)]
                   : Math.round((low + high) / 2);
        // Prefer the single 'recommended' stratification row's midpoint if tagged
        const rec = pr.stratification.find(s => s.recommended || /recommend|target/i.test(s.tag || s.segment || ''));
        const recRange = rec ? parsePriceRange(rec.priceRange) : null;
        pr.targetHomePrice = {
          low,
          mid: recRange ? Math.round((recRange.min + recRange.max) / 2) : mid,
          high,
        };
      }
    }
...
function parsePriceRange(s) {
  ...
  // e.g. "$600-750K" should parse as { min: 600000, max: 750000 }, not 600.
